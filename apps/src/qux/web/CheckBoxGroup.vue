...
export default {
  ...
  methods: {
    isChecked(option) {
      ...
      } else if (this.element?.props?.selected) {
        return this.element?.props?.selected.indexOf(option) > -1
      }
      return false
    },
    validateInput(value) {
  ...
  mounted() {
    ...
    // FIXME: this is not working and messes up the selection
    // the initial value will nto be fired as an array
