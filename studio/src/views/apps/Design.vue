...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      Promise.all([
        ...
        this.modelService.findSecrets(id),
        this.modelService.findDataByApp(id)
      ]).then(values => {
        ...
        let hash = temp[1];
        this.buildCanvas(values[0], values[1], hash, values[3], values[4]);
      });
    },
    buildCanvas(model, stack, hash, secrets, data) {
      ...
      renderFactory.setSourceModel(model);
      ...
      toolbar.initCreateLogic()
      toolbar.initDataList()
      toolbar.initLayerList()



      /**
       * this will also set the model into the 
       * toolbar. The Data.js will set the schema
       * before.
       */
      controller.setModel(model, this.$route.params.sid);

      // needs to be done after model is set
      toolbar.initCreateContent()
    ...
    async initOrg() {
      ...
      AnalyticsService.log("Design", orgID, 'view')
      ...
      if (orgID) {
        if (orgID) {
          ...
          this.updateSelectedOrg(selectedOrg);
        }
      }
    }
  ...
  async mounted() {
    ...
    // FIXME: make something so we get an edit token
