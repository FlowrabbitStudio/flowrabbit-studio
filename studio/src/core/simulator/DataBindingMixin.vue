...
export default {
    ...
    methods: {
		...
		async initDefaultDataBinding (model) {
			...
			/**
			 * Since 5.0 we also support default values
			 */
			if (model.data) {
				this.logger.log(-3, "initDefaultDataBinding","set defaulty values ", model.data);
				this.dataBindingValues = lang.clone(model.data)
			}
			const bindingsCount = {}
