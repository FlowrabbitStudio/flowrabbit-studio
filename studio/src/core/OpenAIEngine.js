...
class OpenAIEngine {
    ...
    constructor() {
        this.proxyURL = `${Services.getConfig().proxy_URL}/proxy` || 'http://localhost:8084/proxy'
    }
    ...
    async query(appID, hash, openAIToken, assistant, content, version, stream) {
        Logger.log(-1, "OpenAIEngine.query() > enter " +  assistant + " > " + content)

        // create thread
        //url, appID, hash, openAIToken, data, version
        const thread = await this.post('https://api.openai.com/v1/threads', appID, hash, openAIToken, undefined, version)
    ...
    streamRun(url, appID, hash, openAIToken, data, version, resolve, reject) {
        const headers = this.createDefaultHeader(url, appID, hash, openAIToken, undefined, version);
        fetch(this.proxyURL + '?stream=true', {
            method: "POST",
            mode: 'cors',
            cache: 'no-cache',
            headers: headers,
    ...
    handleStreamOutput(resolve, reject, response) {
        Logger.log(2, "RestEngine.handleStreamOutput()", "enter", response);
