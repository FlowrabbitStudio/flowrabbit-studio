...
import LogicAnd from 'core/widgets/LogicAnd'
...
import ProgressSegments from 'core/widgets/ProgressSegments'
...
import TabBar from 'core/widgets/TabBar'
import Chat from 'core/widgets/Chat'
import GeoLocation from 'core/widgets/GeoLocation'
import ColumnContainer from 'core/widgets/ColumnContainer'
...
import DataGrid from 'core/widgets/DataGrid'
import AudioPlayer from 'core/widgets/AudioPlayer'
import AudioRecording from 'core/widgets/AudioRecording'
import LabeledImageList from 'core/widgets/LabeledImageList'
import Separator from 'core/widgets/Separator'
import DownloadButton from 'core/widgets/DownloadButton'
...
export default class RenderFactory extends Core {
	...
	setZoomedModel (m) {
		// should be replaced everywhere with source model
		this.zoomedModel = m
	}
	 ...
	 */
	resize(id, pos) {
		const uiWidget = this.getUIWidgetByID(id);
		if (uiWidget) {
			uiWidget.resize(pos);
		} else {
			/**
	...
	_createAudioRecording(parent, model) {
		const widget = this.$new(AudioRecording)
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}
	...
	_createProgressSegments(parent, model) {
		const widget = this.$new(ProgressSegments)
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}

	_createDataGrid(parent, model) {
		const widget = this.$new(DataGrid)
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}

	_createLabeledImageList (parent, model) {
		const widget = this.$new(LabeledImageList)
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}

	_createSeparator (parent, model) {
		const widget = this.$new(Separator)
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}
	...
	_createColumnContainer(parent, model) {
		const widget = this.$new(ColumnContainer);
	...
	_createDownloadButton(parent, model) {
		const widget = this.$new(DownloadButton);
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}

	_createTabBar(parent, model) {
		const widget = this.$new(TabBar);
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}
	...
	_createOpenAIAssistant(parent, model) {
		const widget = this.$new(Rest);
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}

	_createPromptBuilder(parent, model) {
		const widget = this.$new(Rest);
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}

	_createAIRest(parent, model) {
		const widget = this.$new(Rest);
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}

	_createDocToText(parent, model) {
		const widget = this.$new(Rest);
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}

	_createTextToDoc(parent, model) {
		const widget = this.$new(Rest);
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}

	_createDownload(parent, model) {
		const widget = this.$new(Rest);
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}

	_createCopyClipboard(parent, model) {
		const widget = this.$new(Rest)
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}
	
	_createLocalStorage(parent, model) {
		const widget = this.$new(Rest);
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}

	_createFTP(parent, model) {
		const widget = this.$new(Rest);
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}
	...
	_createLogicAnd(parent, model) {
		css.add(parent, "MatcLogic");
		var widget = this.$new(LogicAnd);
		widget.placeAt(parent);
		this._uiWidgets[model.id] = widget;
	}
	...
	setStyle(parent, element, isUpdate = false, isResize = false) {
		let style = ModelUtil.getStyle(element, this.sourceModel);
		if (style) {
			if (!this._uiWidgets[element.id]) {
				/**
				 * Remove this get style???
				 */
				style = ModelUtil.getStyle(element, this.sourceModel);
				if (style) {
					this.setStyle2DomNode(parent, style, element, isResize)
				}
			} else {
				try {
					var w = this._uiWidgets[element.id];
					 ...
					 */
					if (isResize) {
						w.updateScale(element, style, this._scaleX, this._scaleY)
					} else {
						w.render(element, style, this._scaleX, this._scaleY, isUpdate);
					}
	...
	_set_backgroundImage(parent, style, model) {
		...
		if (img) {
			...
			css.remove(parent, "mdi mdi-image");
		} else {
			/**
