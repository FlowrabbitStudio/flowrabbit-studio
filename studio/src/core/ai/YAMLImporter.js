...
export default class YAMLImporter extends HTMLImporter {
    ...
    yamlQuantUX(content, domNode, width, height, options = {}) {
        Logger.log(-1, 'YAMLImporter.yamlQuantUX() > enter', width)
        ...
        const groups = {}
        this.layoutTree(tree, groups, width, this.containerPadding, this.containerPadding)
        const app = this.flattenTree(tree, width, height, groups)     
        ...
        console.debug('cleanedApp', cleanedApp)
    

        return cleanedApp
    }

    layoutTree (node, groups, width, offsetX = 0, offsetY = 0, gapX = 16, gapY = 32, indent= "") {
        ...
        if (this.isRowContainer(node)) {
            ...
            node.children.forEach((child) => {
                ...
                console.debug('layoutTree', child.type, child.w)
                tempOffsetX = child.w + tempOffsetX + gapX
                const offsets = this.layoutTree(child, groups, width, tempOffsetX, tempOffsetY + paddingY, gapX, gapY, indent + "   ")
            ...
            for (let i=0; i < children.length; i++) {
                ...
                this.layoutTree(child, groups, width, tempOffsetX + paddingX, tempOffsetY, gapX, gapY, groups, indent + "   ")
    ...
    getStyle (type, node) {
        ...
        if (type === 'CONTAINER') {
            ...
            result.background = "@panel-background"
            result.borderColor = "@panel-border-color"
            result.boxShadow = "@panel-box-shadow"
            result.borderWidth = "@panel-border-width"
            result.borderRadius = "@panel-border-radius"
            result.padding = 0
        }
    ...
    isInput (node) {
        if (node) {
            ...
            return type === 'TextArea' || type === 'TextBox' || type === 'RadioBox' || type === 'Password' || type === 'Checkbox' || type === 'DateDropDown' || type === 'DropDown' || type === 'RadioGroup' || type === 'CheckBoxGroup'
        }
        return false
    }
}
