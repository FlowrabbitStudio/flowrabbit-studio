...
export default {
  ...
  methods: {
    ...
    getAllAppVariables (){
      if (!this.schema) {
        console.error("SCHEMA NOT DEFINED. FIX THIS")
        console.trace()
        return []
      }

      return SchemaUtil.getAllPaths(this.schema)
      ...
      //         }
      //       }      
      //       this.getRestInputBindings(widget.props.rest).forEach(variable => {
      //         if(variables.indexOf(variable)<0){
      //           variables.push(variable);
      //         }
      //       })
      ...
      //               if(variables.indexOf(databinding)<0 && databinding){
    ...
    getHintsAppVariables (){
        return DataBindingService.getHintsAppVariables(this.model)
		},
     ...
     */
    createZoomedModel(zoomX, zoomY, currentPage) {
      this.logger.log(4, "createZoomedModel", "enter > " + zoomX + " > " + zoomY + " > ", currentPage);
      return CoreUtil.createZoomedModel(zoomX, zoomY, this.model, currentPage)
    },
    ...
    getStartScreen (currentPage) {
      return ModelUtil.getStartScreen(this.model, currentPage)
    },
    ...
    hasCopyClipboard(box) {
      if (box) {
        return box.type == "CopyClipboard";
      }
      return false;
    },
