<template>
						  ...
						  <div class="MatcSimulatorContent" v-if="step === 6">
							  <div class="MatcSimulatorContentCntr">
								  ...
								  <div class="MatcMarginTop">
									  ...
									  <span class="MatcError" v-show="passwordError && passwordError.length > 0" style="margin-left:20px">
										  {{passwordError}}
									  </span>
  ...
  import OpenAIMixing from 'core/simulator/OpenAIMixing'
  ...
  export default {
	  ...
	  mixins:[
		  ...
		  ScrollMixin, AnimationMixin, MouseMixin, DataBindingMixin,OpenAIMixing, TemplateMixin, DojoWidget
	  ],
	  data: function () {
		  return {
			  debug: false,
	  ...
	  methods: {
		  ...
		  doLiveUpdate (app){
			  ...
			  try {
				  this.model = app;
				  this.model = CoreUtil.overwriteScreenSize(this.model, this.currentPage)
				  this.model = this.createZoomedModel(this._scaleX, this._scaleY, this.currentPage);
		  ...
		  setModel (model){
			  ...
			  if (model == null) {
				  ...
				  if (this._splashTime > 0 && this.skipSplash !== true){
					   ...
					   */
					  const t = Math.max(3000 - (new Date().getTime() - this._splashTime),0);
					  setTimeout(() => {
						  this.afterSplash()
					  }, t);
		  ...
		  async startSimilator (model){
			  ...
			  // overwrite screenszie to the selected page
			  //if (!model)
			  this.model = this.createZoomedModel(this._scaleX, this._scaleY, this.currentPage);
			  this.model = Core.addContainerChildrenToModel(this.model);
			  this.model = ModelUtil.inlineTemplateStyles(this.model)
			  ...
			  this.renderFactory.setSourceModel(this.model);
		  ...
		  async executeLogic (screenID, widgetID, widget, orginalLine){
			  ...
			  if (widget.props && widget.props.rest) {
					if (widget.type === 'OpenAIAssistant') {
						restSuccess = await this.executeOpenAIAssistant(widget, orginalLine)
					} if (widget.type === 'PromptBuilder') {
						restSuccess = await this.executePromptBuilder(widget, orginalLine)
					} else {
