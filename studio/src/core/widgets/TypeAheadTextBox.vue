...
<style>
.MatcWidgetTypeChevronIconContainer {
  width: 16px; 
  height: 16px;
  font-size: 12px;
  display: flex;
...
export default {
  ...
  methods: {
    addChevron() {
      ...
      this.chevronNode = db.div("MatcWidgetTypeChevronIcon mdi mdi-chevron-down").build(this.domNode);

      this.chevronNode.style.position = "absolute";
      this.chevronNode.style.right = "15px";
      this.chevronNode.style.top = "50%";
      this.chevronNode.style.transform = "translateY(-50%)";
      this.chevronNode.style.pointerEvents = "none";
    },
    removeChevron() {
      if (this.chevronNode && this.domNode.contains(this.chevronNode)) {
        this.domNode.removeChild(this.chevronNode);
    ...
    onTextBoxRendered(model) {
      ...
      if (this.model?.props?.showChevron) {
        this.addChevron();
      } else {
        this.removeChevron();
      }
    ...
    renderTypeAhead(suggestions) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        const node = db.div("MatcWidgetTypeDropDownOption", option).build(this.popup);
        ...
        node.style.fontSize = this._getBorderWidth(this.style["fontSize"]) + "px";
    ...
    onSimulatorEvent(type, screenID, widgetID) {
      if (type != "ScreenScroll" && type != "Animation" && widgetID != this.model.id) {
        if (this.popup) {
