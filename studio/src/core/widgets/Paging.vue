...
export default {
  ...
  methods: {
    ...
    renderElements(model, style, width, scale) {
      ...
      const cntr = db.div("MatcWidgetTypePagingCntr").build();
      ...
      const elementWidth = this.getZoomed(style.fontSize, scale) * 2
      const elementCount = this.getNumberOfVisibleElements(model, style, width)
    ...
    renderElement(db, width, cntr, value, callback, icon) {
        const element = db.div("MatcWidgetTypePagingElement").w(width).build(cntr);
        ...
        if (icon) {
          const label = db.span("MatcWidgetTypePagingElementLabel").build(element)
          db.span(icon).build(label)
          this._labelNodes.push(label)
        } else {
          const label = db.span("MatcWidgetTypePagingElementLabel", value).build(element)
          this._labelNodes.push(label)
        }
    ...
    resize (box) {
      const elementCount =  this.getNumberOfVisibleElements(this.model, this.style, box.w)
    ...
    setDataBinding (variable, value) {
      ...
      if (databinding && databinding["output"]) {
        const widgetVarialbe = databinding["output"];
      ...
      if (databinding && databinding["elements"]) {
        const widgetVarialbe = databinding["elements"];
        if (widgetVarialbe === variable) {
          this.setMax(value)
          return true;
        }
