...
export default {
  ...
  methods: {
    ...
    render: function(model, style, scaleX, scaleY) {
      ...
      if (this.style.handleShadow) {
        this._set_boxShadowButton(undefined, this.style)
      }
      this.$nextTick(() => {
        if (this.model.props?.checked) {
          this.handle.style.left = `calc(100% - ${this.cntr?.offsetHeight-1}px)`
        } else {
          this.handle.style.left = `-1px`
        }
      })
    },
    ...
    _set_boxShadowButton: function(parent, style) {
      this._setShadow(this.button, style.handleShadow);
    },
