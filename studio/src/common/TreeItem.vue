    ...
    <li >
        <div
         ...
         <label class="MatcTreeItemLabel" v-if="!isEditable" ref="lblNode" >
          ...
          <div class="MatcTreeItemOptions" v-if="value.hasOptions">
            ...
            <span :class="hiddenIcon" @click.stop="toggleHidden" v-if="value.hasHide"></span>
            <span :class="editIcon" @click.stop="onEditItem" v-if="value.hasEdit"></span>
            <span :class="deleteIcon" @click.stop="onDeleteItem" v-if="value.hasDelete"></span>
          </div>
        ...
        <ul v-if="isOpen">
          <TreeItem v-for="child in value.children" :key="child.id"
            ...
            @actionEdit="onChildActionEdit"
            @actionDelete="onChildActionDelete"
...
export default {
  ...
  computed: {
    ...
    hintAndLabel () {
      if (this.value) {
        ...
        if (this.value.defaultValue) {
          let defaultValue  = this.unStripHTML(this.value.defaultValue + '')
          const lbl =  `${this.value.label} (${defaultValue})`
          return this.short(lbl)
        }
    ...
    rowStyle () {
      ...
      if (this.value.selected && this.value.hasChildSelect === false) {
        result += ' MatcTreeItemRowSelected'
      }
      return result
    },
    hasChildren () {
    ...
    isSelected () {
      return this.value.selected && this.value.hasChildSelect !== false
    },
    nodeIcon () {
    ...
    expandIcon () {
      if (this.isOpen) {
        ...
        return 'mdi mdi-chevron-down MatcTreeIcon MatcTreeToggleChildrenIcon'
      }
      if (this.value && this.value.closeIcon) {
         return this.value.closeIcon + ' MatcTreeToggleChildrenIcon'
      }
      return 'mdi mdi-chevron-right MatcTreeIcon MatcTreeToggleChildrenIcon'
    }
  },
  components: {},
  methods: {
    onEditItem (e) {
      this.$emit('actionEdit', this.value.id, e)
    },
    onDeleteItem (e) {
      this.$emit('actionDelete', this.value.id, e)
    },
    unStripHTML(s) {
			if(!s){
				s = '';
    ...
    onChildActionEdit (id, e) {
      this.$emit('actionEdit', id, e)
    },
    onChildActionDelete (id, e) {
      this.$emit('actionDelete', id, e)
    },
    destroy () {
    },
    getInlineTxt (){
    ...
    focus () {
      ...
      if (this.value.editable) {
        setTimeout(() => {
          if (this.$refs.lblNode && this.$refs.lblNode.select) {
