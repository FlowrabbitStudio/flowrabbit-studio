	...
	methods: {
		...
		_sendFiles(e) {
			...
			if (this.isPublic) {
				...
				var hoverScreen = this.getHoverScreen(pos, this.currentPage);
		...
		_onImageUploaded(result, screens, completed) {
			...
			for (let i = 0; i < uploads.length; i++) {
				...
				if (screens[upload.name]) {
					 ...
					 */
					const screen = screens[upload.name];
					...
					if (this._uploadJobs[upload.name]) {
						const job = this._uploadJobs[upload.name];
						const ring = job.ring;
						job.running = false;
						ring.stopAnimation();
						ring.setPercent(1);
		...
		_cleanUpUploadJobs() {
			...
			for (let id in this._uploadJobs) {
				const job = this._uploadJobs[id];
				const ring = job.ring;
				ring.stopAnimation();
				ring.destroy();
				const div = job.div
		...
		_renderUploadJobs() {
			...
			for (let screenID in this._uploadJobs) {
				const job = this._uploadJobs[screenID];
				...
				const cntr = db.div("MatcCenter MatcMiddle").build(div);

				const size = Math.round(Math.min(screen.w, screen.h) * 0.8);
				const ring = this.$new(Ring, {
					color: "#49c0f0",
