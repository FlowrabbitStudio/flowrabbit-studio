...
export default class GridAndRulerSnapp extends Core {
	...
	initLines(absPos) {
		// if this continues to make issues,
		// jsut do not pass the currentPage, so we
		// fall back to scan all. This should work,
		// because the molde here is an inhereted one.
		const screen = this.getHoverScreen(absPos, this.currentPage);
		if (screen) {
