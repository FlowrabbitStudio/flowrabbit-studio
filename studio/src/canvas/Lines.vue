...
export default {
    ...
    data: function () {
        return {
			...
			defaultDataLineColor: "#3787f2",//'#dd1787',
			selectedLineColor: "#dd1787",//'#dd1787',
			defaultLineWidth: 2,
			arrowCorrect : 3,
			arrowSize : 12,
			touchLineWidth: 15,
    ...
    methods: {
			...
			if (fromPos && toPos){
				...
				if (layoutedLine){
					...
					if (!this.lineSVGs[line.id]){
						 ...
						 * create new line
						 */
						const [svg, touchLine, arrow] = this.drawLine(line.id, layoutedLine, line.databinding);
						this.lineSVGs[line.id] = svg;
						this.touchSVGs[line.id] = touchLine
						this.arrowSVGs[line.id] = arrow

						this.wireLine(line, touchLine)
						...
						const touchLine = this.touchSVGs[line.id]
						touchLine.attr("d", this.lineFunction(layoutedLine));
		...
		_lineUpdateUI (){
			if (this._lineUpdateJobs){
				for (let id in this._lineUpdateJobs){
					...
					const touchLine = this.touchSVGs[id]
					if (touchLine) {
						touchLine.attr("d", this.lineFunction(line));
					}
				}
			}
		...
		wireLine(line, touchLine) {
			const listener = on(touchLine, "click", e => {
				this.onLineSelected(line.id, e)
			})
		...
		setLineColor (id, color){
			...
			const arrow = this.arrowSVGs[id]; 
			if (arrow) {
				if (color) {
					arrow.attr("stroke", color);
					arrow.attr("fill", color);
				} else {
					arrow.attr("stroke", this.defaultLineColor);
					arrow.attr("fill", this.defaultLineColor);
				}
			}
		},


		drawLine (id, line, databinding){
			if (databinding) {
				return this.drawSVGLine(id, line, this.defaultDataLineColor, this.defaultLineWidth, 1);
			} else {
		...
		drawSVGLine (id, line, color, width, op){
			...
			const touchGraph = this.svg.append("path")
						.attr("d", this.lineFunction(line))
						.attr("stroke", 'none')
			...
			return [lineGraph, touchGraph, arrow];
		},
