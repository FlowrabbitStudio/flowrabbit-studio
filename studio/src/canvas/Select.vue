    ...
    methods: {
		...
		onWidgetSelected (id, forceSelection = false, ignoreParentGroups = null){
			...
			try {
				if (this.selectionListener) {
					...
					console.warn("onWidgetSelected() > No selection listener")
				}
			} catch (e){
				console.debug(e)
		...
		onWidgetDoubleClick (widgetID) {
			...
			if (widget.type === 'AIRest') {
				if (this.toolbar) {
					this.toolbar.showAIRestDialog(widget)
				}
				return
			}
			if (widget.type === 'OpenAIAssistant') {
				if (this.toolbar) {
					this.toolbar.showOpenAIAssistantDialog(widget)
				}
				return
			}
			if (widget.type === 'PromptBuilder') {
				if (this.toolbar) {
					this.toolbar.showPromptBuilderDialog(widget)
				}
				return
			}
			if (widget.type === 'DocToText') {
				if (this.toolbar) {
					this.toolbar.showDocToTextDialog(widget)
				}
				return
			}
			...
			if (widget.type === 'TextToDoc') {
				if (this.toolbar) {
					this.toolbar.showTextToDocDialog(widget)
				}
				return
			}
			if (widget.type === 'API') {
				if (this.toolbar) {
					this.toolbar.showAPIDialog(widget)
				}
				return
			}
			if (widget.type === 'FTP') {
				if (this.toolbar) {
					this.toolbar.showFTPDialog(widget)
				}
				return
			}
			...
			if (widget.type === 'LocalStorage') {
				if (this.toolbar) {
					this.toolbar.showLocalStorageDialog(widget)
				}
				return
			}
		...
		onLineSelected (id){
			...
			this.setLineColor(id, this.selectedLineColor);
		...
		onCanvasSelected (){
			...
			try {
				...
				const element = document.getElementsByClassName("bb-feedback-button-icon")
				if (element && element.length > 0) {
					css.add(element[0], "display: none");		
				}
			}
		},
