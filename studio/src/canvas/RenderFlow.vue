...
export default {
    ...
    methods: {
		...
		initLineWidgets (model) {
			this._box2Line = {}
			if (model.lines) {
				for (let id in model.lines) {
					const line = model.lines[id]
					if (line) {
						this._box2Line[line.from] = true
						this._box2Line[line.to] = true
					}
				}
			}
		},
		...
		renderFlowViewFast (sourceModel, zoomedModel, isResize = false){
			...
			this.beforeRender();
			this.initLineWidgets(sourceModel)
			...
			this.renderFactory.setSourceModel(sourceModel)
		...
		addLineIndicator (box) {
			let div = this.widgetDivs[box.id]
			if (div) {
				if (this._box2Line[box.id]) {
					css.add(div, 'MatcLineTarget')
				} else {
					css.remove(div, 'MatcLineTarget')
				}
			}
		},
