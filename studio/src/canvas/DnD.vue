...
export default {
  ...
  methods: {
    ...
    _updateWidgetDND (widgetID, pos) {
        ...
        if (dndDiv) {
          ...
          requestAnimationFrame(() => {
            this.domUtil.setPos(dndDiv, pos, true)
          })
        } else {
    ...
    onWidgetDndMove (id, div, pos, dif, e) {
      ...
      if (widget) {
        ...
        if (this.isInContainer(temp)) {
          ...
          // check that logic widgets are not in screens
          if (ModelUtil.mustNotBeInScreen(widget)) {
            const hoverScreen = this.getHoverScreen(temp, this.currentPage)
            if (hoverScreen) {
                this.setDNDError(id, "Logic widget must not be in screen")
            } else {
              this.cleanUpDNDError()
            }
          }
    ...
    onWidgetDndClick (id, div, pos, e) {
      this.logger.log(3, "onWidgetDndClick", "entry > " + id);
