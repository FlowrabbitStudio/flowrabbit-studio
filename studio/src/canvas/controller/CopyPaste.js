...
export default class CopyPaste extends Group{
	...
	modelAddClonedWidgets (ids, pos,selectedGroup, undoIds) {
		...
		const targetScreen = this.getHoverScreen(pos, this.currentPage);
	...
	getClipBoard (){
		...
		if (typeof(Storage) !== "undefined") {
			const str = localStorage.getItem("mactCanvasClipBoard");
	...
	modelPasteClipBoard (clipBoard) {
		...
		clipBoard.widgets.forEach(widget => {
			...
			if (!hasScreen) {
				let parent = this.getHoverScreen(widget, this.currentPage);
				if (parent) {
	...
	onCopyWidgetStyle (source, target){
		...
		const from = this.getBoxById(source, this.currentPage);
		const to = this.getBoxById(target, this.currentPage);
	...
	onMultiCopyWidget (selection, pos){
		 ...
		 */
		const parentPos = this.getBoundingBox(selection);
		...
		const allChildren = this.sortChildren(selection)
	...
	onCopyScreen (id, pos){
		...
		if(screen){
			...
			this.resizePasteObjectsToPage(newScreen, sourcePage)
	...
	resizePasteObjectsToPage (scrn, sourcePage) {
		if (sourcePage === this.currentPage) {
			this.logger.log(-1, 'resizePasteObjectsToPage', 'don nothing')
			return
		}
		if (ModelUtil.isResponsiveModel(this.model)) {
			// FIXME: This could be super responsive !!!
			this.logger.log(-1, 'resizePasteObjectsToPage', 'resize')
			const page = ModelUtil.getScreenSizeByPage(this.model, this.currentPage)
			scrn.w = page.w
			scrn.h = Math.max(scrn.h, page.h)
			scrn.min.w = page.w
			scrn.min.h = page.h

			// update also all widgets according to the grid	
		}
	}
