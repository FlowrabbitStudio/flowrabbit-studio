...
export default class Screen extends CopyPaste {
	...
	updateScreenPosition (id, pos, isMove = false){
		...
		if (screen) {
			...
			if (!screen.segment) {
				...
				this.logger.log(-1,"updateScreenPosition", "screenSize w: ", screenSize.w);
				...
				if (pos.h) {
					pos.h = Math.max(pos.h, screenSize.h);
				}
			} else {
	...
	updateScreenWidthAndHeight (id, pos) {
		...
		if (!screen.segment) {
			const screenSize = ModelUtil.getScreenSizeByPage(this.model, this.currentPage)
			if(pos.w){
				pos.w = Math.max(pos.w, screenSize.w);
			}
			if(pos.h){
				pos.h = Math.max(pos.h, screenSize.h);
			}
	...
	modelScreenSegement (id, isSegment) {
		...
		if (screen) {
			...
			if (isSegment) {
				...
				const screenSize = ModelUtil.getScreenSizeByPage(this.model, this.currentPage)
				screen.min.w = screenSize.w
				screen.min.h = screenSize.h
			}
		}
	...
	updateScreenStart (id, props){
		...
		const screen = this.model.screens[id];
		if (!screen) {
			this.logger.error("updateScreenStart", "No screen > " + id);
			return
		}
		this.startModelChange()

		const pageOfScreen = screen.p
		...
		const oldStart = this.getStartScreen(pageOfScreen);
		if(oldStart){
			delta.o = oldStart.id;
		}

		console.debug("Old start", pageOfScreen, '==', oldStart?.id)

		const command = {
			timestamp : new Date().getTime(),
			type : "ScreenStart",
			delta :delta,
			pageOfScreen: pageOfScreen,
			modelId : id
		};

		this.addCommand(command);
		this.modelScreenStartUpdate(id, props.start, command.pageOfScreen);
		this.commitModelChange()
	}

	modelScreenStartUpdate (id,start, currentPage){
		this.logger.log(-3,"modelScreenStartUpdate", "enter > " + id + " > " + start + " > " + currentPage);

		if (this.model.type === 'responsive') {
		
			for(let s in this.model.screens){
				const screen = this.model.screens[s];
				if (screen.p === currentPage) {
		...
		// do some validation
		let startScreen = this.getStartScreen(this.currentPage);
		if(!startScreen){
	...
	undoScreenStart (command){
		...
		this.modelScreenStartUpdate(command.delta.o, true, command.pageOfScreen);
		this.render();
	}

	redoScreenStart (command){
		this.logger.log(2,"redoScreenStart", "enter > " + command.id);
		this.modelScreenStartUpdate(command.delta.n, command.delta.v, command.pageOfScreen);
		this.render();
	}
	...
	addMultiScreens (screens, fitToBackground){
		...
		let screenCount = this.getScreenCount(this.model, this.currentPage)
		...
		for(let i=0; i< screens.length; i++){
			...
			this._setScreenSizeByPage(screen, true)
			screen.props = {};
			screen.has = {image:true}
			screen.children = [];

			if(fitToBackground){
				const img = screen.style.backgroundImage;
				const scaleRatio = img.w / screen.w;
				const scalledHeight = img.h / scaleRatio;
	...
	addScreensAndWidgets (result, pos){
		...
		if (pos) {
			...
			if (app.widgets) {
				Object.values(app.widgets).forEach(widget => {
					widget.x += pos.x
					widget.y += pos.y
					widget.p = this.currentPage

					const temp = this.getSchemaChanges(widget, widget.props, 'props')
					schemaChanges = schemaChanges.concat(temp)
		...
		this.logger.log(-1,"addScreensAndWidgets", "schemaChanges > ", schemaChanges);
		...
		this.addCommand(command);
		this.modelAddScreenAndWidgets(app);
		if (command.schemaChanges) {
			this.dataApplySchemaChanges(command.schemaChanges)
		}
		this.render();
		this.commitModelChange()
	}
	...
	_createScreenAndWidgets (result){
		...
		const startScreen = this.getStartScreen(this.currentPage);
	...
	undoAddScreenAndWidgets (command){
		...
		const app = command.model;
		
		this.modelRemoveScreenAndWidgets(app);

		if (command.schemaChanges) {
			this.dataApplySchemaChanges(command.schemaChanges, true)
		}
	...
	addScreen (screen, pos){
		...
		// FIXME: Should be also done for copies??
		this._setScreenSizeByPage(screen)

		const count = this.getScreenCount(this.model, this.currentPage)
	...
	getScreenCount (model, currentPage) {
		let count = 0
		if (ModelUtil.isResponsiveModel(model)) {
			...
			return Object.keys(model.screens).length
		}
		return count
	}

	_setScreenSizeByPage (scrn, resize=false) {
		if (this.model.pages && this.model.pages[this.currentPage]) {
			const page = this.model.pages[this.currentPage]
			scrn.min = {
				w : page.w,
				h : page.h
			};
