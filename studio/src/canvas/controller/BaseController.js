...
export default class BaseController extends Core {
  ...
  constructor (params){
		...
		this.currentPage = 'm' // m = mobile, d = desktop
	}
	...
	setPage (page) {
		...
		this.commitModelChange()// hacky way to force render
	}
	...
	setModel (m, screenID){
		...
		this.model = m;
		this.currentPage = ModelUtil.getDefautlPage(m)
		...
		ModelFixer.fixMissingScreenType(this.model)
		ModelFixer.fixDoubleGroup(this.model)
		ModelFixer.fixMissingPage(this.model, this.currentPage)
		ModelFixer.fixDoubleStartScreen(this.model)
	...
	onCanvasSelected (){
		this.logger.log(2,"onCanvasSelected", "enter ");
		if(this.toolbar){
	...
	commitModelChange (updateChangeStack=true) {
		this.logger.log(1,"commitModelChange", "enter  > changes: " + this._modelHasChanged + " > schema:" + this._schemaHasChanged + " > updateChangeStack: " + updateChangeStack);
		const inheritedModel = this.getInheritedModel(this.model)

		if (this._modelHasChanged || this._schemaHasChanged || !updateChangeStack) {
			if (this.toolbar){
			...
			/**
			 * Since 5.0.1 we also add the changes to the command service
			 * As undo() and redo() will call also a save,
			 * we have to set a flag to prevent double saves
			 */
			if (this.oldModel && updateChangeStack) {
				...
				this.addChangeStack(modelChanges, this._schemaChanges)
	...
	getInheritedModel (model) {
		...
		let result =  CoreUtil.createInheritedModel(model)
		// FIXME: this could be done in the createInheritedModel() before doing all the master stuff
		// to shave off some millis?
		result = CoreUtil.filterByPage(result, this.currentPage)
		// we overwrite the screensize so all things in the canvas
		// that use model.screenSize will work correctly
		result = CoreUtil.overwriteScreenSize(result, this.currentPage)
		//console.debug(result)
	...
	async saveModelChanges () {
		this.logger.log(1,"saveModelChanges", "enter > ");
