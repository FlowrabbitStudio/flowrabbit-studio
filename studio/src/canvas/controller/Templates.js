...
export default class Templates extends Command {
		...
		if (newTemplates.length > 0 || removeTemplates.length > 0) {
			...
			instanceGroups.forEach(instanceGroup => {
				if (instanceGroup.id !== group.id) {
					const allChildren = this.getAllGroupChildren(instanceGroup)
					const boundingBox = this.getBoundingBox(allChildren)	
					let targetScreen = this.getHoverScreen(boundingBox, this.currentPage);
	...
	getWidgetTemplatePropsChanges (template, updatedWidget) {
		let result = []
		// for logic widgets, we will replace all rest and api props!
		// if template replacement does not work, check tyhe isLogicMethod, or check the 
		// props to be replaces 
		if (ModelUtil.isLogicWidget(template)) {
			this.logger.log(-1, "getWidgetTemplatePropsChanges", "Update rest");
			if (template.props.rest) {	
				const widgets = ModelUtil.getWidgetsByTemplate(template.id, this.model)
				widgets.forEach(widget => {
					if (widget.id !== updatedWidget.id) {
						result.push({id: widget.id, type:'rest', n: updatedWidget.props.rest, o: widget.props.rest})
					}
				})
			}
			if (template.props.api) {	
				const widgets = ModelUtil.getWidgetsByTemplate(template.id, this.model)
				widgets.forEach(widget => {
					if (widget.id !== updatedWidget.id) {
						result.push({id: widget.id, type:'api', n: updatedWidget.props.api, o: widget.props.api})
					}
				})
			}
		}
	...
	modelUpdateTemplate  (command, render = true) {
		...
		if (command.props) {
			command.props.forEach(r => {
				...
				// 	if (commandWidget.id !== widget.id) {
				const widget = this.model.widgets[r.id]
				const type = r.type
				if (widget && widget.props) {
					widget.modified = new Date().getTime()
					widget.props[type] = r.n
				}
			})
		}
	...
	_createWidgetTemplate (widget, visible, name){
		...
		// templates will not have QSS, because
		// this will mess up a lot of things downstream
		// when resolve
		QSS.replaceWidget(this.model, template)
		return template;
	}
	...
	modelAddTemplate (childTemplates, widgetIDs, template, groupID){
		...
		for (let i=0; i < childTemplates.length; i++) {
			...
			if (widget){
				...
				widget.modified = new Date().getTime()
				widget.style = {};
				// for API widgets we should have this or??
				//widget.props = {};
				if (widget.hover) {
					widget.hover = {}
				}
