...
export default class Widget extends Snapp {
	...
	// 	this.logger.error("updateWidgetDataBinding", "DEPRECATED");
	...
	updateMultiWidgetPosition (positions, fromToolbar, boundingbox, hasCopies){
		...
		if (boundingbox && boundingbox.snapp && boundingbox.type=="boundingbox"){
			...
			if (snapp.type=="All"){
				 ...
				 * Get hover screen
				 */
				const screen = this.getHoverScreen(boundingbox, this.currentPage);
				if(screen) {
	...
	updateWidgetPosition (id, pos, fromToolbar, hasCopies){
		 ...
		 * get the old screen
		 */
		const oldScreen = this.getHoverScreen(widget, this.currentPage);
		 ...
		 * show message
		 */
		const newScreen = this.getHoverScreen(widget, this.currentPage);
		...
		if(newScreen){
			...
			if (ModelUtil.mustNotBeInScreen(widget)) {
				this.showError("Logic widgets are not allowed in screens! Place it on the canvas.");
			}
		} else{
			if(oldScreen){
	...
	modelWidgetUpdate (id, pos, callModelChange = true){
		 ...
		 */
		const screen = this.getHoverScreen(widget, this.currentPage);
		if(screen){
	...
	**********************************************************************/
	...
	createWidgetPropertiesCommand (id, props, type, inlineLabel){
		...
		if (widget){
			...
			// track here creation from the DataBinding dialog
			const schemaChanges = this.getSchemaChanges(widget, props, type)
			const command = {
				timestamp : new Date().getTime(),
	...
	addWidget (model, pos, fromTool){
		this.logger.log(-10,"addWidget", "enter > " + fromTool);
		...
		if (targetScreen) {
			...
			widget.name = this.getCanvasWidgetName(widget.name)
		}
		...
		if(screen){
			...
			if(widget.has && !widget.has.logic){
				this.showError("Great! A new widget was added, but is does not belong to any screen.");
			}
		}
		this.commitModelChange()
		return widget;
	...
	**********************************************************************/
	...
	getReferences (widget){
		...
		for (let widgetID in this.model.widgets) {
			const w = this.model.widgets[widgetID];
			if (w.props && w.props.refs) {
				const refs = w.props.refs;
				for (let refKey in refs){
					const refIds = refs[refKey];
					if (refIds.indexOf(widget.id) >=0){
						const refId = widget.id;
						result.push({
							widget : widgetID,
							refKey : refKey,
							refId : refId
