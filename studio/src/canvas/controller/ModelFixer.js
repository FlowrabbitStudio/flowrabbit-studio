...
class ModelFixer {
	...
	fixMissingPage (model, defaultPage='m') {
		...
		for (let id in model.screens) {
			...
			if (!scrn.p) {
				scrn.p = defaultPage
				errors.push(`scrn: ${scrn.name}`)
			}
		}
		for (let id in model.widgets) {
			const widget = model.widgets[id]
			if (!widget.p) {
				widget.p = defaultPage
				errors.push(`widget: ${widget.name}`)
			}
		}
		...
		if (errors.length > 0) {
			this.logger.warn('fixMissingPage', 'errors', errors.join(','))
		}
	}

	fixDoubleStartScreen(model) {
		const state = {
			m: null,
			d: null
		}
		for (let id in model.screens) {
			const scrn = model.screens[id]
			if (scrn.props.start && scrn.p) {
				if (state[scrn.p]) {
					this.logger.error("fixDoubleStartScreen", "Double start for ", scrn)
				} else {
					state[scrn.p] = scrn.id
				}
	...
	fixMissingScreenType (model) {
		if (model.screens) {
			for (let id in model.screens) {
				const scrn = model.screens[id]
				if (!scrn.type) {
					this.logger.warn("fixMissingScreenType", "No type for ", scrn)
					scrn.type = 'Screen'
				}
			}
		}
		...
		for (let widgetID in model.widgets) {
			...
			if (widget) {
				...
				if (!widgets2Screen[widgetID]) {
					var screen = this._getHoverScreen(widget, model, widget.p)
					if (screen) {
