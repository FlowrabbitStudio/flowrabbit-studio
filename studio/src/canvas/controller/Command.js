...
export default class Command extends BaseController{
    ...
    initChangeStack (id){
        ...
        if (changeStack && Array.isArray(changeStack.stack)) {
            this.logger.log(2,"initChangeStack", "loaded", id);
            this.commandChangeStack = changeStack
        } else {
            this.logger.log(2,"initChangeStack", "create fresh", id);
	...
	addChangeStack (modelChanges, schemaChanges){
        this.logger.log(1,"addChangeStack", "entry > " + modelChanges.length + " > " + schemaChanges?.length);
        ...
        this.commandChangeStack.stack.push({
			...
			schemaChanges: lang.clone(schemaChanges),
			p: this.currentPage
        });
    ...
    undoChangeStack() {
        ...
        if (change) {
			...
			const schemaChanges = change.schemaChanges
			if (schemaChanges) {
				for (let i = 0; i < schemaChanges.length; i++) {
					const c = schemaChanges[i]
					this.applyUndoChange(c, this.data)
				}
    ...
    redoChangeStack() {
        ...
        if (change) {
			...
			const schemaChanges = change.schemaChanges
			if (schemaChanges) {			
				// exceute schema changes and save
				// do not call commit, as this would create some
				// inivite loop
				for (let i = 0; i < schemaChanges.length; i++) {
					const c = schemaChanges[i]
					this.applyRedoChange(c, this.data)
				}
				this.saveSchemaChanges()
				this.saveDefaultDataChanges()
				this.renderSchema()
