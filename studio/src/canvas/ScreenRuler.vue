...
export default {
    ...
    methods: {
        ...
        renderScreenButtons (dndDiv, screen) {
            ...
            /**
             * When a screen segment is deleted, we get not notified!
             */
            //this._renderSegmentScreenLines(screen, dndDiv)
        },

        _renderSegmentScreenLines (scrn, dndDiv){
            if (!this._screenSegmentLines) {
                this._screenSegmentLines = {}
            }
            if (scrn.segment) {
                for(let id in this.model.widgets){
                    const widget = this.model.widgets[id]
                    if (widget.type === 'ScreenSegment') {
                        if (widget?.props?.screenID === scrn.id) {            
                            this._screenSegmentLines[widget.id] = {}
                            if (scrn.h > widget.h) {
                                let handleVertical = document.createElement("div");
                                css.add(handleVertical, " MatcScreenSegmentIndicator MatcScreenSegmentIndicatorVertical");
                                dndDiv.appendChild(handleVertical)

                                handleVertical.style.top = widget.h + 'px'
                                handleVertical.style.left = '0px'
                                handleVertical.style.width =  widget.w + 'px'

                                this._screenSegmentLines[widget.id].handleVertical = handleVertical
                            }


                            if (scrn.w > widget.w) {
                                let handleHorizontal = document.createElement("div");
                                css.add(handleHorizontal, " MatcScreenSegmentIndicator MatcScreenSegmentIndicatorHorizontal");
                                dndDiv.appendChild(handleHorizontal)

                                handleHorizontal.style.top = '0px'
                                handleHorizontal.style.left = widget.w + 'px'
                                handleHorizontal.style.height =  widget.h + 'px'

                                this._screenSegmentLines[widget.id].handleHorizontal = handleHorizontal
                            }
                        }
                    }
                }
        ...
        updateSegmentScreenLines(widget, pos) {
            if (widget.type === 'ScreenSegment') {
                if (this._screenSegmentLines && this._screenSegmentLines[widget.id]) { 
                    const handleHorizontal = this._screenSegmentLines[widget.id].handleHorizontal
                    if (handleHorizontal) {
                        handleHorizontal.style.left = pos.w + 'px'
                        handleHorizontal.style.height = pos.h + 'px'
                    }
                    const handleVertical = this._screenSegmentLines[widget.id].handleVertical
                    if (handleVertical) {
                        handleVertical.style.width = pos.w + 'px'
                        handleVertical.style.top = pos.h + 'px'
                    }    
                }
            }
        },
        ...
        cleanUpSegmentLines() {
            /**
             * FIXME: This is not called. If i add it to 
             * cleanup() in Cnavas.js it get's called at the wrong times...
             * This would cause memeory leaks, iff the screen is deleted or 
             * so.
             * 
             */
            //this.logger.log(3,"cleanUpSegmentLines", "enter");
            delete this._screenSegmentLines
        }
