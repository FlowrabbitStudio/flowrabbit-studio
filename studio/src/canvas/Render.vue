...
export default {
    ...
    methods: {
		...
		showSuccess (msg){
			if (this._currentMessageType === 'error') {
				return
			}
			if(this.message){
		...
		showError (msg){
			if(this.message){
				...
				this.message.textContent = msg;
				this._currentMessageType = 'error'
				setTimeout(lang.hitch(this,"hideMessage"), 3000);
			}
		},
		...
		hideMessage (){
			...
			delete this._currentMessageType
		},
		...
		alignmentStart (selectedType, selectedModel, activePoint, ignoreIds, showDimensions){
			...
			if (this.model.grid) {
				if ("widget" == selectedType || "boundingbox" == selectedType || "group" == selectedType ||  "multi" == selectedType) {
					...
					this._alignmentTool.currentPage = this.currentPage
					if(ignoreIds){
