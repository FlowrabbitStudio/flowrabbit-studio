...
export default {
    ...
    methods: {
		...
		addTemplatedGroup (params){
			...
			const group = this.factory.createTemplatedModel(params);
		...
		_addScreen (params, screen){
			...
			zoomedScreen._isAdding = true
			QSS.replaceVariables(this.sourceModel.theme, zoomedScreen)
		...
		_addWidget (params, widget, mode){
			...
			QSS.replaceVariables(this.sourceModel.theme, zoomedWidget)
			/**
			 ...
			 * add stop listener
			 */
			this._onAddNDropStart(div, widget, params.event, "onWidgetAdded", true);
			this.setState(3);
		...
		addDefaultLine(e, isLineDndStarted) {
			this.logger.log(-1,"addDefaultLine", "enter > " + isLineDndStarted);
			// const selectedWidget = this.getSelectedWidget()
			// if (selectedWidget) {
			// 	this.$refs.lineMenu.show(e, selectedWidget)
			// }	
			this.addLineAtSelected(e, isLineDndStarted)
		},
		...
		addLineAtSelected (e, isLineDndStarted = false) {
			...
			if (selectedWidget && this._lastMouseMoveEvent) {
				...
				/**
				 * For some widgets we might need to return 
				 * the some additional stuff to make it work
				 */
				const actionLines = DataBindingService.getActionTypes(selectedWidget)
				if (actionLines.length > 0) {
					const defaultAction = actionLines[0]
					this.logger.log(-1,"addLineAtSelected", "enter > defaultLineAction: ", defaultAction);
					if (defaultAction.databinding) {
						line.databinding = defaultAction.databinding
					}
					if (defaultAction.targets) {
						line.targets = defaultAction.targets
					}
					if (defaultAction.action) {
						line.action = defaultAction.action
					}
				}
		...
		addLine (params){
			...
			for (let id in this.model.widgets) {
				...
				if (this._addLineStartedFromTemplate) {
					...
					if (widget.type === "LogicAnd") {
						this._addLineActionTargets.push(widget)
					}	
					if (widget.type === "Script") {
						this._addLineActionTargets.push(widget)
					}	
					if (widget.type === "OpenAIAssistant") {
						this._addLineActionTargets.push(widget)
					}		
					if (widget.type === "PromptBuilder") {
						this._addLineActionTargets.push(widget)
					}
					if (widget.type === "AIRest") {
						this._addLineActionTargets.push(widget)
					}
					if (widget.type === "FTP") {
						this._addLineActionTargets.push(widget)
					}
					if (widget.type === "DocToText") {
						this._addLineActionTargets.push(widget)
					}
					if (widget.type === "TextToDoc") {
						this._addLineActionTargets.push(widget)
					}
					if (widget.type === "API") {
						this._addLineActionTargets.push(widget)
					}	
					if (widget.type === "LocalStorage") {
						this._addLineActionTargets.push(widget)
					}
					if (widget.type === "Download") {
						this._addLineActionTargets.push(widget)
					}
					if (widget.type === "CopyClipboard") {
						this._addLineActionTargets.push(widget)
					}
				}
			}
		...
		onLineStartSelected (id, div, pos, e){
			...
			const line = this.factory.createLineModel();
			line.id = "_tempLine";
			line.from = id;
			...
			if (this._addLineParams && this._addLineParams.action) {
				line.action = this._addLineParams.action
			}
			if (this._addLineParams && this._addLineParams.databinding) {
				line.databinding = this._addLineParams.databinding
			}			
			...
			if (line.databinding) {
				this.showHint("Select a widget on the same screen");
			} else {
				this.showHint("Select the screen where the click should go to");
			}
		...
		onLineEndSelected (id, e){
			...
			if (this._addLineModel.databinding) {
				const widget = this.model.widgets[id];
				if (widget) {
					if (!this.isInAllowedTargets(widget)) {
						this.showError("The selected widget is not allowed");
					} else if (!this.hasLogic(widget)) {
						const model = this._addLineModel;
						model.to = widget.id;
						this.controller.addLine(model, e);
						this._onAddDone();
						
					} else {
						this.showError("You can not select a logic widget");
					}

				} else {
					this.showError("You have to select a widget");
				}

			} else if (this._addLineStartedFromTemplate) {
				const widget = this.model.widgets[id];
				if (this.isTemplatedWidgetOnCanvas(widget)) {
				...
				if (screen) {
					...
					if(this.hasLogic(widget)){
						...
						this.controller.addLine(model, e);
						this._onAddDone();
					} else if (this.hasRest(widget) || this.hasScript(widget) || this.hasOpenAIAssistant(widget) 
						|| this.hasPromptBuilder(widget) || this.hasAIRest(widget) || this.hasDocToText(widget)  || this.hasTextToDoc(widget) 
						||  this.hasAPI(widget) || this.hasFTP(widget)|| this.hasLocalStorage(widget) || this.hasDownload(widget) || this.hasCopyClipboard(widget) ) {
						const model = this._addLineModel;
						model.to = widget.id;
						this.controller.addLine(model, e);
						this._onAddDone();
		...
		_updateAddLineMove (e){
			 ...
			 */
			const screen = this.getHoverScreen(to, this.currentPage);
			if (screen && this._addLineModel.databinding) {
				for (let i=0; i < screen.children.length; i++) {
					const widget = this.model.widgets[screen.children[i]]
					if (widget.id !== this._addLineStartId && this.isInAllowedTargets(widget)) {
						if (this._isContainedInBox(to, widget)) {
							to = widget
			...
			} else if(screen && !this._addLineStartedFromTemplate){
				...
				if (this._addLineActionTargets) {
					for (let i=0; i < this._addLineActionTargets.length; i++) {
						...
						if (this._isContainedInBox(to, action) && this.isInAllowedTargets(action)) {
							to = action
			...
			if(!this._addLineSVG){
				const [line] = this.drawLine(this._addLineModel.id, layoutedLine, this._addLineModel.databinding);
				this._addLineSVG = line;
			} else {
		...
		isInAllowedTargets (to) {
			if (this._addLineParams.targets) {
				return this._addLineParams.targets.indexOf(to.type) >= 0
			}
			return true
		},

		getAddLineStartScreen() {
			console.debug('getAddLineStartScreen', this._addLineStartId)
		 ...
		 * AddNDropMethods
		 **********************************************************************/
		_onAddNDropStart (div, model, e, onEndCallback , isMouseUp){
			...
			if (isMouseUp) {
				/**
				 * For the side panel the users can drag and drop or click, and drop.
				 * To check what happened we calculate the that the mouse was moved.
				 */
				const startPos = this.getCanvasMousePosition(e);
				this._addNDropUp = on(win.body(),"mouseup", e => {
					const endPos = this.getCanvasMousePosition(e);
					const divX =  Math.abs(startPos.x - endPos.x)
					const divY = Math.abs(startPos.y - endPos.y)
					if (divX > 16 || divY > 16) {
						this._onAddNDropUp(e);
					} else {
						this.logger.log(-1,"_onAddNDropStart", `Ignore mouse up > divX: ${divX} > divY: ${divY}`);
					}		
				})
			} else {
		...
		_onAddNDropMove (e){
			...
			if(!window.requestAnimationFrame){
				...
				const callback = lang.hitch(this, "_addNDropUpDateUI");
		...
		_addNDropUpDateUI ( ){
			...
			this.domUtil.setPos(this._addNDropNode, this._addNDropNodePos, true)
		},
