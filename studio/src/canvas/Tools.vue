...
export default {
    ...
    methods: {
		...
		onToolRoundStart(e) {
			...
			this.initToolMouseMoveListener("MatcAddRoundTool");
			this._selectionToolUpListener = on(win.body(),"mouseup", lang.hitch(this,"onToolRoundEnd"));
		...
		onToolSeparatorStart(e) {
			...
			this.initToolMouseMoveListener("MatcAddTool");
			this._selectionToolUpListener = on(win.body(),"mouseup", lang.hitch(this,"onToolSeparatorEnd"));
		...
		getPastePostion (mousePos){
			...
			let newScreen = this.getHoverScreen(pos, this.currentPage);

			let oldScreen;
			let boundingBox;
			if (this._copied.widget){
				oldScreen = this.getHoverScreen(this._copied.widget, this.currentPage);
			}
			if (this._copied.multi){
				boundingBox = this.getBoundingBox(this._copied.multi)
				oldScreen = this.getHoverScreen(boundingBox, this.currentPage);
			}
			if(this._copied.group){
				...
				if (this._lastPaste ){
					...
					if (now - this._lastPaste.ts < 20000) {
						...
						if(targetBBX && sourceBBX){
							...
							const targetScreen = this.getHoverScreen(targetBBX, this.currentPage);
							const sourceScreen = this.getHoverScreen(sourceBBX, this.currentPage);
