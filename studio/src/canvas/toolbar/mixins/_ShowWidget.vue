    ...
    methods: {
		...
		****************************************************************************************************/
		...
		showWidgetPrototypeProperties (model) {
			...
			if (widgetViewMode == "style") {
				...
				if (this.hasDataBindingLinks.indexOf(model.type) >= 0 && this.dataLinkDiv) {
					css.remove(this.dataLinkDiv,"MatcToolbarSectionHidden" );
					this.dataBindingLinkWidget.setValue(model, false);
				}
			}

			if (this.condStyleDiv){
				css.remove(this.condStyleDiv, 'MatcToolbarSectionHidden')
				this.condStyle.setValue(model)
			}
		...
		showWidgetDesignProperties (model) {
			...
			// some widget, like the data section go on the direct model.style
			const qssModel = lang.clone(model)
			qssModel.style = style
			...
			if(model?.has?.label || model?.has?.text){
			...
			if(model?.has?.label || model?.has?.color){
			...
			if (widgetViewMode == "style") {
				...
				if((this.hasData && this.hasData.indexOf(model.type) >=0) || model?.has?.data) {
					css.remove(this.dataDiv,"MatcToolbarSectionHidden" );
					// @FIXME: pass here the style as well
					this.dataWidget.setValue(qssModel);
				}
			...
			if (this.hasData && this.hasData.indexOf(model.type) >= 0 
				|| (this.hasDataHover && this.hasDataHover.indexOf(model.type) >= 0 )
				|| (this.hasDataActive && this.hasDataActive.indexOf(model.type) >= 0 )
				|| (this.hasDataFocus && this.hasDataFocus.indexOf(model.type) >= 0)
			) {
				css.remove(this.dataDiv,"MatcToolbarSectionHidden" );
				this.dataWidget.setValue(qssModel, false, widgetViewMode);
			} 
		...
		*/
		getViewModeStyle (widget, widgetViewMode){
			...
			if (theme) {
				widget = QSS.replaceVariables(theme, lang.clone(widget))
			}
