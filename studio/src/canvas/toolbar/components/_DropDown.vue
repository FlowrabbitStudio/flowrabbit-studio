...
<template>
     <div class="MatcToolbarItem ">
		<div type="button" data-dojo-attach-point="button">
			...
			<span class="mdi mdi-chevron-down" data-dojo-attach-point="caret"></span>
			</div>
			<div :class="['MatcToolbarPopUp', placement ? `MatcToolbarPopUp--${placement}` : '']" role="menu" data-dojo-attach-point="popup">
		</div>
	</div>
</template>
<script>
...
export default {
    ...
    methods: {
		...
		_reposition (forceUpdatePosition = false) {
			...
			var h = win.getBox().h;
			if(pos.y > h * 0.667){
				this.popup.style.top = pos.y - (this.popupPos.h - pos.h -10) + "px"
			} else if (pos.y > h* 0.33){
				this.popup.style.top = pos.y - (this.popupPos.h / 2.5) + "px"
			} else {
		...
		renderArrow () {
			if (this.arrowPosition) {
				...
				if (!this.arrow) {
					...
					let arrowClass = "MatcToolbarPopUpArrowCntr";
					if (this.placement) {
						arrowClass += ` MatcToolbarPopUpArrowCntr--${this.placement}`;
					}
					css.add(this.arrow, arrowClass);
					let triangle = document.createElement("div");
					let triangleClass = "MatcToolbarPopUpArrow";
					if (this.placement) {
						triangleClass += ` MatcToolbarPopUpArrow--${this.placement}`;
					}
					css.add(triangle, triangleClass);
