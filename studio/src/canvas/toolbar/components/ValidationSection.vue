...
export default {
    ...
    methods: {
		...
		_showButton (model){
			// The data binding would be for a variable
			// this._setSectionLabel('Button')
			// this._renderDataBinding(model);

			//this._renderSubSection("Loading Animation");
			this._setSectionLabel('Loading Animation')
			this._renderLabelDropDown("loadingAnimation", model, "loadingAnimation", [
				{ value: null, icon: "DeleteX", label: "None" },
				{ value: "loading", icon: "Loading1", label: "With Label" },
				{ value: "loadingIcon", icon: "Loading2", label: "Without Label" },
			]);
			if (model?.props.loadingAnimation) {
				this._renderSubSection("Loading Label");
				this._renderInput(model.props, "loadingText", "", 'Loading Label');
			}
		...
		_showLabel (model){
			...
			this._renderSubSection('Content')
			this._renderTextArea(model.props, 'label', 'Label value', 'Enter a message')
			this._renderSubSection('Formatting')

			const options = [
				{ value: "none", label: "None" },
				{ value: "markup", label: "Rich Text" },
				{ value: "json", label: "JSON" }
			]
			if (!model.props.format) {
				model.props.format = "none";
			}
			this._renderRadioGroup(options, model.props.format, "format")
		...
		_showChat(model) {
			this._renderDataBinding(model);
			this._renderCheck("Allow Markdown",model.props.markup, "markup" );
		},
