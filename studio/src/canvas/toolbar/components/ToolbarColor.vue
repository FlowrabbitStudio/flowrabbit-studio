...
<template>
	<div class="MatcDesignTokenMixin">
		<div
			:class="[' MatcToolbarItem MatcToolbarColor', { 'MatcToolbarGridFull': hex }, { 'MatcToolbarLabeledColor': label }, { 'MatcToolbarColorHexError': hexError }]">
			<div id="colorPaletteId" type="button" data-dojo-attach-point="button" class="MatcToolbarColorButton">
				...
				<input v-if="hex" class="MatcIgnoreOnKeyPress  MatcToobarInput" @mousedown.stop="" @click.stop="focusHex"
					:value="colorAsHex" @change="setColorHasHex" ref="hexInput" />
			</div>
		</div>
		<div class="MatcToolbarPopUpBackDrop" v-if="isDialog && isOpen" @click="hideDropDown"></div>
		<div class="MatcToolbarPopUp MatcToolbarDropDownButtonPopup" role="menu" data-dojo-attach-point="popup"></div>
	</div>
</template>
<script>
...
export default {
	...
	methods: {
		...
		_renderColorWidgets(popup) {
			...
			const cntr = db.div("MatcToolbarPopUpContainer MatcToolbarColorContainer MatcToolbarColorSketch").build();

			/**
			 * render tabs
			 */
			const tabHeader = db.div("MatcToolbarTabContainer").div("MatcToolbarTabs").build(popup);

			/**
			 * Picker Tab
			 */
			if (this.hasPicker) {
				const tabPicker = this.createTab("Solid", tabHeader, cntr, db, true);
			 ...
			 */
			if (this.hasGradient) {
				const tabGradient = this.createTab("Gradient", tabHeader, cntr, db, false);
		...
		_renderThemeColors(div) {
			const db = new DomBuilder()
			const themeColors = []
			const theme = this.model.theme
			for (let key in theme) {
				if (theme[key].type === 'color') { // filter for subType?
					themeColors.push({
						key: key,
			...
			if (div) {
				div.innerHTML = "";
				css.add(div, 'MatColorThemeCntr')
				themeColors.forEach(color => {
					const row = db.div('MatColorThemeRow').build(div)
					db.span("MatcColorBox").span().background(color.value).build(row)
					db.span("MatcToolbarColorThemeLabel", this.getNLS('themes.' + color.key)).build(row)
					this.tempOwn(on(row, touch.press, lang.hitch(this, "onChange", color.key)));
				})
			}
		},
		...
		_renderCustomColors(customColors, labelDiv, div) {
			if (div) {
				...
				if (customColors.length > 0) {
					const customColorBoxes = this.renderColorBoxes(customColors, this.customColorDiv, this.columns, "onColorSelected");				
		...
		getBoxGradient(boxes, result) {
			for (let id in boxes) {
				const box = boxes[id];
				const back = box.style.background;
		...
		setGradient(gradientPicker, d) {
			const gradient = gradientPicker.getValue();
			if (gradient) {
		...
		createTab(lbl, parent, cntr, db, showDefaultColor) {
			...
			const tab = db.a("MatcToolbarTabActive", lbl).build(parent);
			const tabCntr = db.div().build(cntr);
			this.tabs.push(tab);
