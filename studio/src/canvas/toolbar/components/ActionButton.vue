...
export default {
	...
	methods: {
		...
		setValue(widget, isLogicWidget) {
			...
			const actions = DataBindingService.getActionTypes(widget)
			if (actions.length > 0) {
				this.renderMultiAction(db, parent, widget)
			} else if (isLogicWidget) {
				this.renderLogic(db, parent, widget)
			} else {
				this.renderClassic(db, parent, widget)
			}
			this.domNode.appendChild(parent);
		},

		renderClassic(db, parent, widget) {
			...
			if (!line && !action) {
				...
				let btn = this.$new(ToolbarDropDownButton, { maxLabelLength: 20, hasCaret: false  });
				btn.setLabel('<span class="mdi mdi-link-variant "></span><span class="MatcButtonIconLabel">Add Link</span>');
		...
		renderMultiAction(db, parent, widget) {
			const lines = this.getFromLines(widget);
			const allActions =  DataBindingService.getActionTypes(widget)
			const icons = {}
			const availableOptions = allActions.filter(a => {
				icons[a.action] = a.icon
				const matchedLined = lines.filter(l => l.action === a.action)
				return matchedLined.length === 0
			})
			const linesWithAction = lines.filter(line => {
			...
			const cntr = db.div('MatcToolbarDataBindingLinks').build(parent)

			for (let i = 0; i < linesWithAction.length; i++) {
				const line = linesWithAction[i];
				const to = this.getToBox(line);	
				const icon = icons[line.action]

				const item = db.div('MatcToolbarItem MatcToolbarIconButton MatcToolbarIconButtonPassive').build(cntr)
				db.span('MatcToolbarItemIcon').span(icon).build(item)
			...
			if (availableOptions.length > 0) {
				...
				btn.setLabel('<span class="mdi mdi-link-variant "></span><span class="MatcButtonIconLabel">Add Link</span>');
				btn.updateLabel = false;
				btn.setOptions(availableOptions);
				btn.setPopupCss("MatcMultiActionDropDownPopup");
				btn.reposition = true;
				css.add(btn.domNode, "MatcMultiActionDropDown MatcToolbarButton MatcButton");
				btn.placeAt(row);
				btn.on("change", (id, e, line) => {
					console.debug(id, e, line)
					this.onNewActionLine(e, line.action, line.databinding, line.targets)
				})
				// btn.setShowListener(() => {
				// 	topic.publish("matc/canvas/fadeout", {});
				// });
				...
				//db.div("MatcToolbarItem MatcToolbarGridFull", "No more links to set").build(parent);
			}
		...
		renderNewSchool(db, parent, line, isWidget) {
			...
			console.debug("renderNewSchool() > line", this.getEventTypes(line, isWidget, btn));
		...
		getEventTypes(line, isWidget, btn) {
			...
			if (this.widget && (this.widget.type == "TextBox" || this.widget.type == "Password" || this.widget.type == "LabeledTextBox")) { //
				return [
					{ value: "click", label: "Click", icon: "mdi mdi-cursor-default" },
			...
			if (this.widget && (this.widget.type == "Chat")) { //
				return [
					{ value: "click", label: "Change", icon: "mdi mdi-contain-end" }
				]
			}
		...
		showEditWorkflow(action, e) {
			...
			var settings = this.$new(WorkflowSettings, {schema: this.schema});
		...
		onNewActionLine(e, action, databinding, target) {
			this.emit("newLine", e, action, databinding, target);
		},
		...
		onLineActionByID(id, value) {
			console.debug('onLineActionByID', id, value)
			this.emit("setLinePropertyByID", id, "action", value);
		},
		...
		onEditRule(line, e) {
			try {
				...
				this.stopEvent(e);
				const db = new DomBuilder();
				const popup = db.div(" MatcPadding").build();
				const cntr = db.div("").build(popup);


				const rule = this.$new(Rule);
				rule.setModel(this.model);
				rule.setSchema(this.schema);
				rule.setScreenIDs(this.getScreenIDs());
				rule.setValue(line);
				rule.placeAt(cntr);

				const bar = db.div("MatcButtonBarDialog MatcMarginTop").build(popup);
