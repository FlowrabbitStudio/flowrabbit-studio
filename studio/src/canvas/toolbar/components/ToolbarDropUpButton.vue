<template>
  <div :class="['MatcToolbarPopUpCntr', { MatcToolbarIconDropDownButton: isIconButton }, { MatcToolbarArrowDropDown: hasArrow }]">
    <div type="button" ref="button" class="MatcToolbarItem MatcToolbarDropDownButton MatcToolbarDropUpButton MatcToolbarIconButton">
      <QIcon :icon="icon" v-if="icon"></QIcon>
      <label data-dojo-attach-point="label" class="MatcHidden MatcToolbarItemIcon"></label>
      <span class="mdi mdi-chevron-down" ref="caret"></span>
    </div>
    <div class="MatcToolbarPopUpBackDrop" v-if="isDialog && isOpen" @click="hideDropDown"></div>
    <div class="MatcToolbarPopUp MatcToolbarDropDownButtonPopup" role="menu" data-dojo-attach-point="popup" :class="['MatcToolbarPopUp--' + placement]">
      <div class="MatcToolbarPopUpWrapper">
        <ul class="" role="menu" data-dojo-attach-point="ul"></ul>
      </div>

      <div class="MatcToolbarPopUpArrowCntr" v-if="hasArrow" :class="['MatcToolbarPopUpArrowCntr--' + placement]">
        <div class="MatcToolbarPopUpArrow" :class="['MatcToolbarPopUpArrow--' + placement]"></div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    if (o.label || o.icon || o.css || o.value != null) {
      ...
      css.add(contentWrapper, "MatcToolbarPopUpContentWrapper");
      ...
      if (o.label) {
        ...
        contentWrapper.appendChild(lbl);
      ...
      if (o.label && o.shortcut) {
        var scSpan = document.createElement("span");
        css.add(scSpan, "MatcToolbarPopUpShortcut");
        scSpan.innerHTML = o.shortcut;
      ...
      if (o.callback) {
        ...
        this.tempOwn(on(li, touch.press, lang.hitch(this, "onChange", o.value, o)));
      }
  ...
  return selectedValue;
    ...
    // Always clear out whatever was there
    if (this.label) {
      this.label.innerHTML = "";
    }

    // If there's no value at all, hide the label
    if (!value) {
      this.label && css.add(this.label, "MatcHidden");
      return;
    }

    // Otherwise, ensure label is shown

    // If we have an array of objects with icons and labels
    if (this.hasObjects) {
      // Clear label text again
      this.label && (this.label.innerHTML = "");
      const o = this._options?.find((o) => o.value == value);
      if (o) {
        // Set icon if it exists on the chosen option
        if (o.icon) {
          this.icon = o.icon;
        }
        // If there's a label and we want text, display it
        if (o.label && this.hasLabelTxt && this.label && !this.hideSelectedLabel) {
		  this.label && css.remove(this.label, "MatcHidden");
          const lbl = document.createElement("span");
          css.add(lbl, "MatcToolbarDropDownButtonLabel");
          let displayText = o.label;
          if (displayText.length > this.maxLabelLength) {
            displayText = displayText.substring(0, this.maxLabelLength) + "...";
          }
  ...
  mounted() {
    ...
    if (this.hasCaret === false || this.qHideCaret) {
      this.hideCaret();
    }
    if (this.qIcon) {
    ...
    if (this.qPlacement) {
      this.setPlacement(this.qPlacement);
    }
  },
};
</script>
