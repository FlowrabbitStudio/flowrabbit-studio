...
<template>
	<div class=" MatcToolbarPopUpCntr MatcToolbarImage ">
		...
		<div class="MatcToolbarPopUp" role="menu" data-dojo-attach-point="popup">			
			<div class="MatcImageUpload" data-dojo-attach-point="upload" @click.stop="" @mousedown.stop @mouseup.stop           @keydown.stop="" 
			@keyup.stop="" >
				<div data-dojo-attach-point="imageContainer" class="MatcImageUploadContainer MatcImageUploadContainerXL">
				</div>
	...
	methods: {
		...
		render(data) {
			...
			for (let i = 0; i < data.length; i++) {
				...
				css.add(wrapper, "MatcImageUploadPreviewWrapper")
				...
				css.add(text, 'MatcImageUploadPreviewText MatcIgnoreOnKeyPress form-control')
				wrapper.appendChild(text)
				this.textNodes[url] = text

				const found = this.selection.find(s => s.url == url);
				if (found) {
					text.value = found.text					
				}
				this.tempOwn(on(text, "change", () => {
					const s = this.selection.find(s => s.url == url);
					if (s) {
						s.text = text.value
					} else {
						this.selection.push({
				...
				this.tempOwn(on(text, touch.press, (e) => {
					e.stopPropagation();
				}));

				this.tempOwn(on(text, "mousedown", (e) => {
					e.stopPropagation();
				}));
		...
		imageIsSelected(img) {
			if (this.multiSelection) {
				var url = img.url;
				return this.selection && this.selection.findIndex(s => s.url == url) >=0;
			} else {
		...
		_selectImage(v, div, e) {
			...
			if (this.multiSelection) {
				...
				const url = v.url;
				const pos = this.selection.findIndex(s => s.url == url);
				if (pos >= 0) {
					this.selection.splice(pos, 1);
					css.remove(div, "MatcImageUploadPreviewSelected");
					this.textNodes[url].value = ""
				} else {
					this.selection.push({
						url: url,
						text: ''
