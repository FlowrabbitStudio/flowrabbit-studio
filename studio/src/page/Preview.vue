...
export default {
  ...
  props: ['app', 'screen', 'isPublic', 'token', 'pub', 'isFillBackground', 'invitation'],
  data: function() {
    return {
      debug: false,
      mode: "standalone",
  ...
  methods: {
    ...
    setModel (model, screenID, widgetID) {
      this.logger.log(2,"setModel", "enter > m:" + model.id + " > s:" + screenID + " > w:" + widgetID);

      this.model = model;
      if (!this.currentPage) {
        this.currentPage = ModelUtil.getDefautlPage(model, true)
      }
      this.model = ModelUtil.overwriteScreenSize(model, this.currentPage)
      this.selectedWidgetID = widgetID;
      if (this.model.fonts) {
				this.attachFontsToDom(this.model.fonts)
      }
      this.initScale();
      this.model = this.createZoomedModel(this._scaleX, this._scaleY, this.currentPage, true);
      /**
    ...
    render (screen) {
      ...
      if (!screen) {
        screen = this.getStartScreen(this.currentPage);
      }
  ...
  mounted() {
    if (this.app) {
      this.postCreate()
      if (this.pub) {
        this.setPublic(this.pub);
      }
      if (this.token) {
        this.setJwtToken(this.token)
      }
      if (this.invitation) {
        this.setInvitation(this.invitation)
      }
