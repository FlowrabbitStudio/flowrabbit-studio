...
class DocParser {
  ...
  async parseDocx(req, res) {
    ...
    try {
      ...
      try {
        ...
        const documentFile = zip.files.find(d => d.path === "word/document.xml");
        if (!documentFile) {
          throw new Error("Unable to find 'word/document.xml' in the DOCX file.");
        }
  ...
  convertToDOCx(res, __dirname, uploadsDir, inputPath) {
    ...
    const referenceDocxPath = path.join(__dirname, "utils/custom-reference.docx");
    if (!fs.existsSync(referenceDocxPath)) {
      console.error(`Reference DOCX template not found at ${referenceDocxPath}`);
    ...
    try {
      ...
      if (Array.isArray(parsedData)) {
        if (parsedData.length > 0 && typeof parsedData[0] === "object") {
          ...
          records = parsedData.map(item => headers.map(header => item[header]));
        } else {
          records = parsedData;
        }
  ...
  async convertToExcel(markdownString, res, __dirname, uploadsDir, inputPath) {
    ...
    try {
      ...
      const lines = markdownString.split("\n");
      records = lines.map(line => line.split(",").map(cell => cell.replace(/\r/g, "")));
    }
  ...
  async converToDoc(req, res, __dirname) {
    ...
    try {
      if (docFormat === "docx") {
        ...
        await this.convertToCSV(markdownString, res, __dirname, uploadsDir, inputPath);
      } else if (docFormat === "excel" || docFormat === "xlsx") {
        await this.convertToExcel(markdownString, res, __dirname, uploadsDir, inputPath);
      } else if (docFormat === "pdf") {
        await this.convertToPDF(markdownString, res, __dirname, uploadsDir, inputPath);
      }
    } catch (error) {
      console.error("Failed to process Doc:", error);
      res.status(500).json({
        message: "Failed to process Doc",
        error: error.toString()
      });
    }
  }
