...
public class UserRESTTestCase extends BaseTestCase {
	...
	private JsonObject findUser(TestContext context, User user) {
		...
		context.assertFalse(u.containsKey("error"));
		context.assertFalse(u.containsKey("password"));
		context.assertTrue(u.containsKey("email"));
		return u;
	}

	private JsonObject findUserError(TestContext context, User user) {
		JsonObject u = this.get("/rest/user/" +  user.getId() +".json");
		context.assertTrue(u.containsKey("error"));
		return u;
	}
	...
	@Test
	public void testSpam(TestContext context){
		log("testSpam", "enter");
		...
		User klaus = postUser("klaus", context);
		assertLogin(context, "klaus@quant-ux.de", "123456789");
		JsonObject partical = new JsonObject().put("spam", new JsonArray().add("message1").add("message2"));
		this.post("/rest/user/"  + klaus.getId() + ".json", partical);


		JsonObject partical2 = new JsonObject().put("spam", new JsonArray().add("message1").add("message2").add("message3"));
		this.post("/rest/user/"  + klaus.getId() + ".json", partical2);
		...
		context.assertTrue(loadedUser.containsKey("spam"));
		context.assertEquals(3, loadedUser.getJsonArray("spam").size());
	...
	public void testHack(TestContext context){
		...
		JsonObject partial = new JsonObject().put("role", User.ADMIN);
		this.post("/rest/user/"  + klaus.getId() + ".json", partial);
		...
		context.assertTrue(loadedUser.containsKey("_id"));
		context.assertTrue(!loadedUser.containsKey("password"));
		// role should be still user
		context.assertEquals(User.USER, loadedUser.getString("role"));
