...
public class AppRestTest4 extends BaseTestCase {
	...
	public void testAppCopy(TestContext context){
		...
		// check data
		JsonObject dataCopy = this.getData(newId, context);
		context.assertTrue(dataCopy.containsKey("schema"));

		// check data
		JsonObject secretsCopy = getAppSecrets(newId, context);
		context.assertTrue(secretsCopy.containsKey("secrets"));
		context.assertEquals(2, secretsCopy.getInteger("version"));

		// check DB counts
		context.assertEquals(2l, client.count(DB.getTable(AppSecrets.class), Model.all()));
		context.assertEquals(2l, client.count(DB.getTable(AppData.class), Model.all()));
