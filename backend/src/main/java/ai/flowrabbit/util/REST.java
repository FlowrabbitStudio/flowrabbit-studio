...
public abstract class REST {
	...
	protected User getHashUser(RoutingContext event) {
		String hash = getId(event, "hash");
        return TokenService.getUser(hash);
	}
	...
	protected void returnJson(RoutingContext event, JsonObject result){
		if (result == null) {
			logger.error("returnJson() > Null passed");
	...
	protected void returnError(RoutingContext event, String error, int status){
		JsonObject result = new JsonObject().put("type", "error");
		result.put("error", new JsonArray().add(error));
		event.response().setStatusCode(status);
		event.response().putHeader("content-type", "application/json");
		event.response().end(result.encodePrettily());
	}
