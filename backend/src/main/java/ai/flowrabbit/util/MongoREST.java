...
public class MongoREST extends CrudREST {
	...
	protected boolean isPartialUpdate = true;
		...
		if (!user.hasRole(User.USER)) {
			...
			mongo.find(team_db, Team.findByUser(user), res ->{
				if (res.succeeded()) {
					...
					logger.error("getACLList() > Could not read ACL");
	...
	public void setPartialUpdate(boolean enablePartialUpdate) {
		this.isPartialUpdate = enablePartialUpdate;
	}
	...
	protected void update(RoutingContext event, String id, JsonObject json) {
		...
		if(this.isPartialUpdate){
			partialUpdate(event, id, json);
		} else {
			completeUpdate(event, id, json);
		}
	}

	protected void partialUpdate(RoutingContext event, String id, JsonObject json) {
	...
	protected void completeUpdate(RoutingContext event, String id, JsonObject json) {
		...
		mongo.save(table, json, res -> {
			if(res.succeeded()){
				...
				returnError(event, "complete.update.error");
			}
		});
	}
	...
	protected void returnUpdateAppPart(RoutingContext event, String appID) {
