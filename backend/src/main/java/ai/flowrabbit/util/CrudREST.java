...
import ai.flowrabbit.validation.Validator;
...
public abstract class CrudREST extends REST {
	...
	protected abstract void getACLList(User user, Handler<String> handler);
	...
	private void createAllowed(RoutingContext event) {
		...
		if(json!=null){
			if (this.validator != null) {
				this.validator.validate(event, json, true, err -> {
					if (err.isEmpty()) {
			...
			this.acl.canWrite(getUser(event), event, allowed -> {
				if (allowed) {
					...
					this.getACLList(user, userApps -> {
						...
						MongoLogger.error(event, getUser(event), this.getClass(), "update", "Update without permission. ACL: " + userApps);
					});
	...
	public void update(RoutingContext event, String id) {
		...
		if(json!=null){
			if(this.validator !=null){
				this.validator.validate(event, json, false, new Handler<List<String>>() {
			...
			this.acl.canRead(getUser(event), event, allowed -> {
				if (allowed) {
					...
					this.getACLList(user, userApps -> {
						...
						MongoLogger.error(event, getUser(event), this.getClass(), "find", "Read without permission. ACL: " + userApps);
					});
					returnError(event, 401);
				}
			});
			...
			this.acl.canRead(getUser(event), event, allowed -> {
				if (allowed) {
					...
					this.getACLList(user, userApps -> {
						...
						MongoLogger.error(event, getUser(event), this.getClass(), "findByACL", "Read without permission. ACL: " + userApps);
					});
					returnError(event, 401);
				}
			});
			...
			this.acl.canRead(getUser(event), event, allowed -> {
				if (allowed) {
					...
					this.getACLList(user, userApps -> {
						...
						MongoLogger.error(event, getUser(event), this.getClass(), "countByAcl", "Read without permission. ACL: " + userApps);;
					});
