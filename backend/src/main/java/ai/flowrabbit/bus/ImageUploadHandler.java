...
public class ImageUploadHandler implements Handler<Message<JsonObject>>{
	...
	public void handle(Message<JsonObject> event) {
		...
		String resize = msg.getString("resize");
		...
		client.findOne(app_db, App.findById(appID), App.getFields("screenSize"), res->{
			...
			if(res.succeeded()){
				...
				if(app!=null){
					int maxWidth = getScreenWidth(app, resize);
	...
	private int getScreenWidth(JsonObject app, String resize) {
		if ("false".equals(resize)) {
			return Image.MAX_IMAGE_NO_RESIZE_WIDTH;
		}
