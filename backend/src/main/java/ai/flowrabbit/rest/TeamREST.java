...
public class TeamREST extends MongoREST {
    ...
    private final String user_db, team_db, app_db, pub_settings_db, org_team_db;
    ...
    public Handler<RoutingContext> createPermission() {
        return event -> validateAppPermission(event);
    }


    private void validateAppPermission(RoutingContext event) {
        /**
    ...
    private void validateUser(RoutingContext event, String appID) {
                ...
                mongo.findOne(user_db, User.findByEmail(email), null, res -> {
                    ...
                    if (res.succeeded()) {
                        ...
                        if (user != null) {
                             ...
                             */
                            validateOrgPermission(event, appID, user, permission);
                        } else {
                            /**
    ...
    private void validateOrgPermission(RoutingContext event, String appID, JsonObject user, int permission) {

        mongo.findOne(pub_settings_db, PublicationSettings.findByApp(appID), null, res -> {
            if (res.succeeded()) {
                JsonObject settings = res.result();
                if (settings == null || Organization.DEFAULT_ID.equals(settings.getString(PublicationSettings.FIELD_ORG_ID))) {
                    createPermission(event, appID, user, permission);
                } else {
                    // validate that the current user can also access the org
                    String orgID = settings.getString(PublicationSettings.FIELD_ORG_ID);
                    String userID = user.getString("_id");
                    this.organizationACL.canRead(userID, orgID, allowed -> {
                        if (allowed) {
                            createPermission(event, appID, user, permission);
                        } else {
                            returnError(event, "apps.team.member.add.error.not.in.org");
                        }
                    });
                }
            } else {
                logger.error("validateOrgPermission() Cannot read pub settings");
                returnError(event, 405);
            }
        });
    }
    ...
    private void createPermission(RoutingContext event, String appID, JsonObject user, int permission) {
        ...
        if (permission > 0) {
			...
			String userID = user.getString("_id");
            JsonObject team = Team.create(userID, appID, permission);

            mongo.count(team_db, Team.findByUserAndApp(userID, appID), found -> {
                if (found.succeeded() && found.result() == 0) {
                    /**
                     * Only insert if no user is found.
                     */
                     ...
                     * Return error
                     */
                    logger.error("createPermission() > User already created in " + appID);
                    returnError(event, 404);
                }
            });
        ...
        mongo.findOne(app_db, App.findById(appID), null, appFound -> {
            if (appFound.succeeded()) {
                ...
                Mail.to(user.getString("email"))
                        .subject(domain.getUrl() + " - A prototype was shared with you")
                        .payload(payload)
    ...
    private void getSuggestion(RoutingContext event) {
        ...
        User user = getUser(event);
        if (User.USER.equals(user.getRole()) || User.ADMIN.equals(user.getRole())) {
