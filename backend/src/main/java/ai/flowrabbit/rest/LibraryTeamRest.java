...
import ai.flowrabbit.acl.LibraryAcl;
import ai.flowrabbit.bus.MailHandler;

import ai.flowrabbit.model.AppEvent;
import ai.flowrabbit.model.Library;
import ai.flowrabbit.model.LibraryTeam;
		...
		mongo.findOne(lib_db, Library.findById(libID), null, libFound->{
			if(libFound.succeeded()){
				...
				Mail.to(user.getString("email"))
					.subject("Quant-UX - A library was shared with you")
					.payload(payload)
