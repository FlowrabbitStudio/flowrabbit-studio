...
import ai.flowrabbit.validation.AppValidator;
...
public class AppREST extends MongoREST {
	...
	private final PreviewService preview = new PreviewService();
	
	private final String team_db, inv_db, image_db, pub_settings_db, app_data_db, app_secret_db;
	
	//private final String imageFolder;

	private final OrganizationACL organizationACL;

	private final IBlobService blobService;
	/**
	 * Default AppREst with users ACL
	 */
	public AppREST(MongoClient db, String folder, IBlobService blobService) {
		this(db, folder, new RoleACL(new AppAcl(db)).read(User.GUEST), blobService);
	}
	
	private AppREST(MongoClient db, String folder, Acl acl, IBlobService blobService) {
		super(db, App.class);
		this.setACL(acl);
		...
		this.app_secret_db = DB.getTable(AppSecrets.class);
		this.organizationACL = new OrganizationACL(db);
	...
	private void copy(RoutingContext event, String appID, String name, JsonObject oldApp, List<JsonObject> images) {
		...
		mongo.save(table, fixedApp, res->{
			if(res.succeeded()){
				...
				this.createImageFolder(event, oldApp, images, fixedApp, done -> {
					this.copyData(appID, id);
					this.copySecrets(appID, id);
				});
	...
	private void copySecrets(String oldID, String newID) {

		this.mongo.findOne(app_secret_db, AppData.findByApp(oldID), null, res-> {
			if (res.succeeded() && res.result()!= null) {
				JsonObject secrets = res.result();
				JsonObject copy = secrets.copy();
				copy.put(AppSecrets.APP_ID, newID);
				copy.remove("id");
				copy.remove("_id");

				this.mongo.save(app_secret_db, copy, saved -> {
					if (!saved.succeeded()) {
						logger.error("copySecrets() Could not save");
					}
				});

			} else {
				logger.error("copySecrets() Could not find");
			}
		});
	}

	private void copyData(String oldID, String newID) {

		this.mongo.findOne(app_data_db, AppData.findByApp(oldID), null, res-> {
			if (res.succeeded() && res.result()!= null) {
				...
				copy.remove("id");
				copy.remove("_id");
				this.mongo.save(app_data_db, copy, saved -> {
					if (!saved.succeeded()) {
						logger.error("copyData() Could not save");
					}
				});
			} else {
				logger.error("copyData() Could not find");
			}
		});
	}
	...
	private void createImageFolder(RoutingContext event, JsonObject oldApp, List<JsonObject> images, JsonObject newApp, Handler<Boolean> done){
		String newID = newApp.getString("_id");
		String newFolder = blobService.createFolder(event, newID);
		try{
			if(newFolder != null){
				copyImages(event, images, newApp, done);
			} else {
	...
	private void copyImages(RoutingContext event, List<JsonObject> images, JsonObject newApp, Handler<Boolean> done) {
		...
		for(JsonObject image : images){
			...
			mongo.save(image_db, newImage, imageResult ->{
				...
				if(imageResult.succeeded()){
					blobService.copyBlob(event, oldUrl, newUrl, fileResult -> {
						if(!fileResult){
							this.logger.error("copyImages() > Could not copy image" + oldUrl + " > " + newUrl);
						} else {
	...
	protected void afterCreate(RoutingContext event, JsonObject app) {
		...
		JsonObject owner = Team.create(getUser(event), appID, Acl.OWNER);
		...
		if (orgID != null && !orgID.isEmpty()) {
			User user = getUser(event);
			this.organizationACL.canCreateApp(user, event, allowed -> {
				if (allowed) {
					JsonObject settings = PublicationSettings.createInstance(appID, orgID);
					mongo.save(pub_settings_db, settings, settingsCreated -> {
						if (settingsCreated.succeeded()) {
							logger.debug("afterCreate() > Settings created");
						} else {
							logger.error("afterCreate() Could create settings");
						}
					});
				} else {
					logger.error("afterCreate() > User " + user.getEmail() + "  tried to create app in wrong org " + orgID);
				}
			});
		}
	...
	private void deleteAppFolder(RoutingContext event, String appID) {
		log("deleteAppFolder", "Enter "+ appID);
		blobService.deleteFolder(event, appID);
//		String appFolder = imageFolder + "/" + appID;
//		FileSystem fs = event.vertx().fileSystem();
...
//					if (!exists.succeeded()) {
//						User user = getUser(event);
//						error("deleteAppAndParts", "Could not delete path " + appFolder + " " + deleted.cause());
//						MongoLogger.error(event, user, AppREST.class, "deleteAppAndParts", appID);
//					} else {
//						log("deleteAppAndParts", "Deleted path " + appFolder);
//					}
//				});
//			}
//		});
	...
	private void applyChanges(final RoutingContext event) {
		...
		acl.canWrite(getUser(event), event, allowed ->{
			...
			if(allowed){
				...
				MongoLogger.error(event,getUser(event), this.getClass(), "applyChanges","Write to app " + appID + " without permission");
	...
	public Handler<RoutingContext> findEmbedded() {
		return event -> findEmbedded(event);
	}
	...
	private void findEmbedded(RoutingContext event) {
		...
		this.acl.canRead(getUser(event), event, allowed -> {
			if (allowed) {
				...
				MongoLogger.error(event,getUser(event), this.getClass(), "findEmbedded","Read without permission");
				returnError(event, 401);
			}
		});
